 ...
 * would eventually fall by moving right, then move right. Otherwise, rise at it's current position.Here, 
 * "eventually fall" means that the droplet will eventually be at a lower level if it moves in
 ...
 * Output: [2,3,3,4]
 * Explanation:
 * The last droplet settles at index 1, since moving further left would not cause it to 
 * eventually fall to a lower height.
 * Example 3:
 * 
 ...
 * Note:
 * 
 * heightsÂ will have length inÂ [1, 100]Â and contain integers inÂ [0, 99].
 * VÂ will be in rangeÂ [0, 2000].
 * KÂ will be in rangeÂ [0, heights.length - 1].
 */

class Solution {
	
	// Time 0(N*V) constant space
    public int[] pourWater(int[] heights, int V, int K) {
    	for(int i = 0;i < V;i++) {
    		int l = findleft(heights, K);
    		if(l != K) {
    			heights[l]++;
    		} else {
    			int r = findright(heights, K);
    			if(r != K)
    				heights[r]++;
    			else 
        			heights[K]++;
...
public class Main {
	public static void main(String[] args) {
		...
		int V = 4, K = 3;
		new Solution().pourWater(heights, V, K);
	}
}
